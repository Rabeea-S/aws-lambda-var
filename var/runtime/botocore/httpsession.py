...
try:
    from urllib3.util.ssl_ import PROTOCOL_TLS, OP_NO_TICKET, PROTOCOL_TLS_CLIENT
except ImportError:
    ...
    from ssl import PROTOCOL_TLS, OP_NO_TICKET, PROTOCOL_TLS_CLIENT

    from distutils.version import LooseVersion
    from urllib3 import __version__ as urllib3_version
    if LooseVersion(urllib3_version) < LooseVersion('1.25.4'):
        import warnings
        _WARNING_OUTDATED_URLLIB3 = (
            "Your function includes an outdated version of `urllib3` (version: '" + urllib3_version + "'). "
            "This dependency version is no longer compatible with Botocore "
            "(ref. https://github.com/boto/botocore/blob/b00276641e31e14f79e996aef769d8b0509467fe/setup.cfg), "
            "and is affected by at least the following CVEs: CVE-2020-26137 (High), CVE-2021-33503 (Med). \r"
            "Lambda has included a custom layer to preserve backwards compatibility, which is why this message is being logged. "
            "This compatibility layer is put in place as a temporary mitigation for functions that see this warning, and will be removed from Lambda. "
            "To remove the risk of these CVEs and to stop seeing this warning, please take one of the following actions: \r"
            "1. Update the version of `urllib3` included in your function's deployment package.\r"
            "2. Remove the packaged version of `urllib3` in order to make use of the more recent version included in the Lambda Runtime."
        )
        warnings.filterwarnings(
            action="once",
            category=UserWarning,
            module=__name__,
            message=_WARNING_OUTDATED_URLLIB3,
            append=True,
        )
        warnings.warn(message=_WARNING_OUTDATED_URLLIB3, category=UserWarning, stacklevel=2)
